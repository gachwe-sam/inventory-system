...
<form action="{{ route('branches.destroy', $branch) }}" method="POST" class="d-inline">
    @csrf @method('DELETE')
    <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Delete this branch and all of its subbranches?')">
        <span class="loading-dots d-done"></span><i class="bi bi-trash"></i>
    </button>
</form>
